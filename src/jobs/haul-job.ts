...
import { load as loadRemotable, isRemotableSource, RemotableEnergyStore, REMOTABLE_TYPE_SOURCE, RemotableContainer, RemotableStorage, RemotableSpawn, RemotableExtension, RemotableTower } from "../remotables/remotable";
...
export class HaulTarget extends JobTarget<StructureContainer | StructureStorage | StructureSpawn | StructureExtension | StructureTower, RemotableContainer | RemotableStorage | RemotableSpawn | RemotableExtension | RemotableTower> {
    constructor(site: RemotableContainer | RemotableStorage | RemotableSpawn | RemotableExtension | RemotableTower, public remainingTransferAmount: number) { super(site); }
    ...
    get targets(): HaulTarget[] {
        if (this._targets === undefined) {
            ...
            for (let i = 0; i < this.creep.memory.job.targets.length; i++) {
                ...
                let site = <RemotableContainer | RemotableStorage | RemotableSpawn | RemotableExtension | RemotableTower | undefined>loadRemotable(target.savedSite, [STRUCTURE_CONTAINER, STRUCTURE_STORAGE, STRUCTURE_SPAWN, STRUCTURE_EXTENSION, STRUCTURE_TOWER]);
